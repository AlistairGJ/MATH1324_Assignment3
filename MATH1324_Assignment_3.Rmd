...
All Diet / Gym Contain 2 outliers, will just use Coles Adjusted to find (with > 120)
...
```{r}
supplement_filterAdjusted <- dsAdjusted$`Allocated Category` == "Supplement"
supplement_dataAdjusted <- dsAdjusted[supplement_filterAdjusted,]
```

```{r}
summary(supplement_dataAdjusted$WoolworthsAdjusted)
```
...
t.test(dsAdjusted$ColesAdjusted,dsAdjusted$ColesSaleAdjusted)
```
